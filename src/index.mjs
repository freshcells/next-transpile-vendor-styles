/**
 * Source: https://github.com/martpie/next-transpile-modules
 * License: https://github.com/martpie/next-transpile-modules/blob/master/LICENSE
 * License: https://github.com/freshcells/next-transpile-vendor-styles/blob/main/LICENSE
 *
 * Adjusted to only transpile stylesheets
 */
...
const createLogger = (enable) => {
  return (message, force) => {
    if (enable || force) console.info(`next-transpile-vendor-styles - ${message}`)
  }
}
...
const withTmInitializer = (modules = [], options = {}) => {
  return (nextConfig = {}) => {
    ...
    const resolve = enhancedResolve.create.sync({
      ...
      extensions: ['.esm.js', '.js', '.jsx', '.ts', '.tsx', '.mjs', '.css', '.scss', '.sass'],
    ...
    const getPackageRootDirectory = (module) => {
      try {
        ...
        throw new Error(
          `next-transpile-vendor-styles - an unexpected error happened when trying to resolve "${module}". Are you sure the name of the module you are trying to transpile is correct, and it has a package.json with a "main" or an "exports" field?\n${err}`
        )
      }
    }
      ...
      webpack(config, options) {
        ...
        if (nextCssLoaders) {
          ...
          if (nextGlobalCssLoader) {
            ...
            console.warn(
              'next-transpile-vendor-styles - could not find default CSS rule, global CSS imports may not work'
            )
          }

          const nextGlobalSassLoader = nextCssLoaders.oneOf.find(
            (rule) =>
              rule.sideEffects === true && regexEqual(rule.test, /(?<!\.module)\.(scss|sass)$/)
          )
          ...
          if (nextGlobalSassLoader) {
            ...
            console.info(
              'next-transpile-vendor-styles - global SASS imports only work with a custom _app.js file'
            )
          }
        }
